...
def structure_fix(input_ids: list, structure_one: str, vocabulary: dict) -> int:
...
def function_structure(input_ids: list, vocabulary: dict, name_function: list) -> int:
    logits = np.array(model.get_logits_from_input_ids(input_ids))
    mask = np.full(len(logits), float('-inf'))
    valid_ids = []
...
def type_number(input_ids: list, vocabulary: dict):
    logits = np.array(model.get_logits_from_input_ids(input_ids))
    mask = np.full(len(logits), float('-inf'))
...
def type_string(input_ids: list, vocabulary: dict):
    logits = np.array(model.get_logits_from_input_ids(input_ids))
    mask = np.full(len(logits), float('-inf'))
    valid_ids = [token_id for token_id, token_str in vocabulary.items() if ',' not in token_str and '}' not in token_str and '{' not in token_str]
    mask[valid_ids] = logits[valid_ids]
    next_token = np.argmax(mask)
    return next_token
...
if __name__ == '__main__':
    ...
    str_function = str(name_and_decription(_function))
    ...
    structure = []
    first_structure = f"{{\"prompt\": \"{prompt[9]['prompt']}\",\"name\": \""
    ...
    tensor = model.encode(f"{func_str} Give me the name of description : that responds to this prompt: {prompt[9]}. The output should be structured like this: {{'prompt': '','name': , 'parameters': {{}}}}")
                ...
                if key_or_value == "value":
                    ...
                    if type_arg == 'string':
                        ...
                        if i == len_arguments:
                            ...
                            input_ids.append(int(next_token))
                            structure.append(int(next_token))
                    print(model.decode(structure))
                    break

    with open('output.txt', 'w') as files:
        files.write(model.decode(structure))

    dict_ = json.loads(model.decode(structure))
    result_final = []
    result_final.append(dict_)
    with open('output.json', 'w') as files:
        json.dump(result_final, files, indent=4)
